...
import { useAccess as useIdentity } from "@/lib/access";
import { useI18n } from "@/lib/i18n";

type Access = { loading: boolean; allowed: boolean; isAdmin: boolean };

/**
 * useAccess answers, for one permission, what this caller may do with it.
 *
 * It is a shape over lib/access, which holds the one identity request the whole
 * shell shares — asking again per screen would have meant a /auth/me for every
 * component that wanted to hide a button.
 *
 * Administrators bypass the permission check server-side and come back with an
 * empty permission list, so the flag is honoured separately: otherwise an admin
 * would be shown a read-only screen.
 */
export function useAccess(permission?: string): Access {
  const identity = useIdentity();
  return {
    loading: !identity.ready,
    isAdmin: identity.isAdmin,
    allowed: identity.isAdmin || !permission || identity.can(permission),
  };
}
