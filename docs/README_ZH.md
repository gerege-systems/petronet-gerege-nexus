...
PetroNet System **不是一个应用，而是一个生态系统**。司机、加油站、燃料企业与监管机构
做的是四种不同的工作，每一方都有为这份工作而建的平台。在它们之下，共享同一份数据、
同一套身份和同一条审计链。
...
| 平台 | 面向谁 | 承载什么 |
| --- | --- | --- |
| 公民平台 | 司机与公民 | 最近的加油站、牌号、库存水平、每日额度与代金券。身份来自 eID |
| 企业门户 | 燃料企业 | 油库与加油站登记、油罐余量，以及各报告期的报送与更正流程 |
| 监管指挥中心 | AMGTG 与检查人员 | 全国日汇总、可用天数、覆盖缺口、对账差异与告警 |
| 加油站 POS 与边缘代理 | 加油站 | 油枪、油罐、班次、支付与票据，基于本地数据库：断网时销售照常 |
| 数据仓库与分析 | 分析人员与管理层 | 每日从操作系统抽取、清洗、按业务建模，并提供 BI 与预测 |
| 控制台与可观测性 | 运营人员 | 组织、权限与审计位于独立登录之后，旁边是指标、告警与备份 |

它们之下是同一个 Go 二进制、同一个数据库、同一套身份：这里的生态系统指的是多个工作
空间，而不是多台服务器。对外则连接海关、国家登记、e-Barimt、eID 以及现场的液位仪。
